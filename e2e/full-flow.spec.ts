...
test('PWA 外壳声明独立启动、竖屏策略、图标和安装说明', async ({ page, request }, testInfo) => {
  ...
  await expect(guide).toContainText('只保存在当前设备')
  await expect(guide).toContainText('Safari 网页与主屏幕应用的存储可能分开')
  await expect(guide).toContainText('定期')
  await expect(guide).toContainText('JSON 备份')
